...
def stop_stream(task_id: str, reason: str = "用户请求停止") -> bool:
    """停止视频流任务"""
    logger.info(f'开始停止推流任务 - task_id={task_id}, reason={reason}')
    
    with process_lock:
        if task_id not in active_processes:
            logger.warning(f'任务不存在 - task_id={task_id}')
            # 即使任务不在活动进程中，也将状态设置为stopped而不是error
            if reason != 'auto_stop':  # 只有手动停止时才设置为stopped
                update_task_status(task_id, {
                    'status': 'stopped',
                    'end_time': datetime.now(beijing_tz).isoformat(),
                    'message': '任务已手动停止（任务可能已结束或不存在）'
        ...
        try:
            ...
            with process_lock:
                if task_id in active_processes:
                    ...
                    # 记录视频文件状态
                    video_path = active_processes[task_id].get('video_path')
                    if video_path and os.path.exists(video_path):
                        file_size = os.path.getsize(video_path)
                        file_time = datetime.fromtimestamp(os.path.getmtime(video_path))
                        task_logger.info(f"视频文件状态: 大小={file_size}字节, 最后修改时间={file_time}")
                    else:
                        task_logger.warning(f"视频文件不存在或无法访问: {video_path}")
                    
                    # 记录代理文件状态（如果使用了代理）
                    proxy_config = active_processes[task_id].get('proxy_config')
                    if proxy_config and os.path.exists(proxy_config):
                        task_logger.info(f"代理配置文件存在: {proxy_config}")
                        
                    # 记录网络统计数据
                    if 'buffer_success_count' in active_processes[task_id]:
                        task_logger.info(f"累计网络自动恢复次数: {active_processes[task_id]['buffer_success_count']}")
                    
                    if 'buffer_timeout_count' in active_processes[task_id]:
                        task_logger.info(f"累计网络缓冲超时次数: {active_processes[task_id]['buffer_timeout_count']}")
...
def stop_stream_sync(task_id: str):
    ...
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        result = loop.run_until_complete(stop_stream(task_id, reason='auto_stop'))
        loop.close()
